...
make test          # run all tests (over 400; zero real distrobox needed)
...
- **List (home)**: table of boxes (NAME | STATUS | IMAGE | DOCKER | CBOX?) with brand header and state badges; auto-refreshes (~2s) so box state updates live. Arrow keys to move, `c` to create, `s` to stop, `d` to destroy, `b` for bulk operations, `a` to apply, `e` to edit, `enter` to inspect or enter (if running), `D` for doctor, `q` to quit.
  - **Overlays & modes:**
    - **Fuzzy filter** (`/`): type to narrow the box list live; `Enter` keeps selection, `Esc` clears.
    - **Command palette** (`:`): fuzzy-searchable list of actions; type to narrow, `Enter` runs.
...
**Test coverage:**
- **Over 400 tests**, all mock-driven.
...
**Tests use mocks.** All tests (over 400) run without any real distrobox installed. Real-host validation happens via optional smoke tests (`#[ignore]`, manual run).
